...
#«»±˖˗‑‐‒ ━ ✚‐↔←↑↓→↭⇎⇔⋆━◂▸◄►◆☀★☗☊✔✖❮❯⚑⚙
  ...
  if $(git rev-parse --is-inside-work-tree >/dev/null 2>&1); then
    ...
    echo -n "$fg_no_bold[$fgclr]${ref/refs\/heads\//$PL_BRANCH_CHAR $upstream_prompt}${mode}$to_push$to_pull$clean$tagged$stashed$untracked$modified$deleted$added$ready_commit$fg_no_bold[$fgclr]"
  fi
}
...
prompt_dir() {
  ...
  FILE_FOLDE='\U1F4C1'
  OPEN_FILE_FOLDE='\U1F4C2'
  HAMMER_AND_SICKLE='\U262D'
  prompt_segment cyan white "$fg_no_bold[black]${OPEN_FILE_FOLDE}%~$fg_no_bold[white]"
}
...
prompt_time() {
  # the default content of '/etc/localtime' is '../usr/share/zoneinfo/Asia/Shanghai'
  # normal content of '/etc/localtime' should be '/usr/share/zoneinfo/Asia/Shanghai'
  OldTimeZone=$(readlink /etc/localtime |awk -F '/usr/share/zoneinfo/' '{print $2}')
  NewTimeZone='Asia/Shanghai'
  if [[ "${OldTimeZone}" != "${NewTimeZone}" ]]; then
    unlink /etc/localtime
    ln -s /usr/share/zoneinfo/${NewTimeZone} /etc/localtime
  fi

  # the format of current time is just like:
  # Friday 2016-12-23 18:43:46 CST
  prompt_segment black yellow "$(TZ="${NewTimeZone}" date '+%A %Y-%m-%d %H:%M:%S') $(date +%Z)"
}
...
preexec() {
  cmd_timestamp=$(timer_now)
}

precmd() {
  local stop=$(timer_now)
  local start=${cmd_timestamp:-$stop}
  let last_cmd_exec_time=$stop-$start
  unset cmd_timestamp
}
...
prompt_status() {
  local symbols
  symbols=()
  WHITE_SMILING_FACE='\U263A'
  WHITE_FROWNING_FACE='\U2639'

  if [[ $RETVAL -eq 0 ]]; then
    lastcode="%{%F{green}%}$RETVAL"
    symbols+="%{%F{green}%}✔"
    # symbols+="%{%F{green}%}${WHITE_SMILING_FACE}"
  else
    lastcode="%{%F{red}%}$RETVAL"
    symbols+="%{%F{red}%}✘"
    # symbols+="%{%F{red}%}${WHITE_FROWNING_FACE}"
  fi

  [[ $UID -eq 0 ]] && symbols+="%{%F{yellow}%}⚡"
  [[ $(jobs -l | wc -l) -gt 0 ]] && symbols+="%{%F{cyan}%}⚙"

  # HOURGLASS_WITH_FLOWING_SAND='\U023F3'
  CLOCK_FACE_THREE_OCLOCK='\U1F552'

  [[ -n "$symbols" ]] && prompt_segment blue white "%{%F{cyan}%}($(display_time ${last_cmd_exec_time})) ${lastcode} ${symbols}"
}
...
prompt_context() {
  prompt_segment magenta white "$USER@%m"
}
